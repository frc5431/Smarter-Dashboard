...
public class RobotDashboard {
	...
	public static void main(String[] args) {
		...
		Jumbo.setMainaction(()->{
			final JumboScene scene = new JumboScene();
			
			final Rectangle connectionbounds = new Rectangle(750,50,1410,50);
			final JumboImage connection = new JumboImage(new JumboTexture(JumboColor.YELLOW), connectionbounds);
			scene.addEntity(connection);
			final JumboTextBox connectiontext = new JumboTextBox(connectionbounds, new JumboText("Starting up..."));
			scene.addEntity(connectiontext);
			
			Jumbo.setScene(scene);
		});
		Jumbo.start(new JumboLaunchConfig("Test", new Dimension(2160,784),new BufferedImage[]{SmarterDashboard.getImage("res" + File.separator + "logo.png")},"res"+File.separator+"liavishis"));
